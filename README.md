...
This work presented a comprehensive end-to-end semantic segmentation framework for the assistive robot ROSWITHA, spanning the full pipeline from raw annotated data to real-time robotic deployment via ROS2. By extending the perception system from 7 to 10 classes — adding laptops, backpacks, and microwaves — the project broadened ROSWITHA's ability to understand and interact with complex indoor environments, which is essential for assistive tasks such as object retrieval, environment monitoring, and autonomous navigation. The data preparation stage proved critical to the system's effectiveness. Converting JSON polygon annotations into dense pixel-wise masks, incorporating depth channels (X, Y, Z) alongside RGB into a 6-channel input tensor, and serialising into TFRecord format established a reliable, efficient foundation for supervised learning. The multi-modal representation allowed the FRA-UAS-SS model to exploit both visual appearance and spatial geometry, resolving ambiguities that neither modality could handle alone.
 
The proposed U-Net-based architecture, combining an FC-HarDNet backbone with MMAF-Net fusion and SE attention, demonstrated strong performance on dominant classes. The combined focal and Dice loss guided optimisation under class imbalance, and the conservative fine-tuning strategy — developed through 14 systematic sessions — found the best achievable balance between rare-class learning and dominant-class stability. The automated safety checks that halted training when any class dropped below 65% proved essential in preventing catastrophic forgetting. The successful deployment within ROS2 Humble validated the practical feasibility of integrating deep learning perception into a real-time robotic system. The modular three-node architecture enabled scalable operation at 15 Hz on GPU hardware, meeting ROSWITHA's perception requirements. However, the results also expose a fundamental limitation: for classes representing less than 0.01% of training pixels, no combination of loss weighting, oversampling, or architectural attention can fully compensate for data scarcity. The mIoU drop from 0.7270 (7-class) to 0.575 (10-class) underscores this reality. Future work should prioritise expanding the dataset for underrepresented classes through targeted collection and synthetic generation, adopting transformer-based architectures for improved small-object detection, and applying continual learning techniques like Elastic Weight Consolidation to mitigate forgetting. These improvements, combined with the robust pipeline established here, provide a solid foundation for advancing intelligent robotic perception toward fully autonomous assistive systems.
